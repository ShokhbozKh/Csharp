...
IF SCHEMA_ID(N'Expense') IS NULL EXEC(N'CREATE SCHEMA [Expense];');

GO

IF SCHEMA_ID(N'Income') IS NULL EXEC(N'CREATE SCHEMA [Income];');
...
CREATE INDEX [IX_Expense_CategoryId] ON [Expense].[Expense] ([CategoryId]);

GO

CREATE INDEX [IX_Income_CategoryId] ON [Income].[Income] ([CategoryId]);
